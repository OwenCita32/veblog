<nav class="bg-gray-800" x-data="{ isOpen: false }">
    ...
    <div class="flex h-16 items-center justify-between">
        ...
        <div class="ml-4 flex items-center md:ml-6">
          ...
          <div class="relative ml-3">
            <div>
              @if (Auth::check())
                <button type="button" @click="isOpen = !isOpen"
                  ...
                  <img class="size-8 rounded-full"
                    src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : asset('img/avatar_default.png') }}"
                    alt="{{ Auth::user()->name }}">
                  <div class="text-gray-300 font-semibold ml-3">{{ Auth::user()->name }}</div>
                  <div class="ms-1">
                    <svg class="text-gray-300 font-semibold fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 20 20">
              ...
              @else
                <a href="/login" class="text-white font-semibold text-sm">Login</a>
                <span class="text-white font-semibold">|</span>
                <a href="/register" class="text-white font-semibold text-sm">Register</a>
              @endif
            </div>
      ...
      <div class="-mr-2 flex md:hidden">
        ...
        <button type="button" @click="isOpen = !isOpen"
          ...
          <!-- Menu untuk open: "hidden", Menu bukan open atau closed: "block" -->
          <svg :class="{ 'hidden': isOpen, 'block': !isOpen }" class="size-6" fill="none" viewBox="0 0 24 24"
            stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
          </svg>
          <!-- Menu untuk open: "block", Menu bukan open atau closed: "hidden" -->
  ...
  <div x-show="isOpen" class="md:hidden" id="mobile-menu">
    ...
    <div class="border-t border-gray-700 pt-4 pb-3">
      ...
      @else
        <a href="/login"
          class="block text-white font-semibold rounded-md px-3 py-2 text-base hover:bg-gray-700 hover:text-white">Login</a>
        <a href="/register"
          class="block text-white font-semibold mt-3 rounded-md px-3 py-2 text-base hover:bg-gray-700 hover:text-white">Register</a>
      @endif
    </div>
  </div>
</nav>
